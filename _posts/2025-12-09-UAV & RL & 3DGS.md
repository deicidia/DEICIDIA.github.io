...
Cette fois-ci, j'ai tenté un scan à plus grande échelle en filmant l'intégralité de mon appartement, mais l'absence de GPU NVIDIA a provoqué des erreurs. Sous Windows, COLMAP a échoué en raison d'un nombre excessif de features. 
```bash
Creating SIFT GPU feature matcher
E: ough GPU memory to match 6365 features. Reduce the maximum number of matches.
```
Sous WSL2, le résultat fut limité : seules 23 images ont été conservées pour l'entraînement sur brush.

![Visualisation Zig](/images/appart.png)
...
En raison d'un bug référencé [ici](https://github.com/colmap/glomap/issues/221), je ne peux pas l'utiliser pour l'instant. Toutefois, notez que le code nécessite l'utilisation de COLMAP pour le feature matching et le matching spatial.
