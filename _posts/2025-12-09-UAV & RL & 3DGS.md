...
> **Note :** Sur la page du scan, double-cliquez sur le modèle pour une manipulation plus intuitive.
...
En appliquant cette logique sur une vidéo du Fuji XT-2 ([voir le scan](/scans/fuji.html)), j'ai utilisé `ffmpeg` pour extraire 1 image par seconde :

```bash
ffmpeg -i input_video.mp4 -vf fps=1 out%d.png
```
